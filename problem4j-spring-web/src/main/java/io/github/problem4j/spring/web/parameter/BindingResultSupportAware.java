 ...
 * interface is detected by {@link BindingResultSupportAwareBeanPostProcessor} and configured with
 * the container's {@link BindingResultSupport} bean.
 *
 * @see BindingResultSupport
 * @since 3.1.0
 */
@FunctionalInterface
public interface BindingResultSupportAware {

  /**
   * Called with the container's configured {@link BindingResultSupport} after this bean has been
   * constructed.
   *
   * @param bindingResultSupport the binding result support to use
   * @since 3.1.0
   */
  void setBindingResultSupport(BindingResultSupport bindingResultSupport);
}
